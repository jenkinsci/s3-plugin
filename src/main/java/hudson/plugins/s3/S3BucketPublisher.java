            ...
            for (Entry entry : entries) {
                ...
                for (FilePath src : paths) {
                    log(listener.getLogger(),
                            "bucket=" + bucket + ", file=" + src.getName() + " region=" + selRegion +
                            ", upload from slave=" + entry.uploadFromSlave + " managed="+ entry.artifactManagement +
                            " , server encryption "+entry.useServerSideEncryption
                    );
                    records.add(
                            profile.upload(
